output "tailscale_subnet_router_public_ip" {
  value = values(module.subnet_router).*.public_ip
}

output "tailscale_subnet_router_private_ip" {
  value = values(module.subnet_router).*.private_ip
}

output "tailscale_app_connector_public_ip" {
  value = values(module.app_connector).*.public_ip
}

output "tailscale_app_connector_private_ip" {
  value = values(module.app_connector).*.private_ip
}
